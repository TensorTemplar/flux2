...
@click.option("--remote", "-r", default=None, help="Use remote server (e.g., http://localhost:8000)")
def main(
    model_name: str | None,
    prompt: str | None,
    seed: int | None,
    ...
    if upsample is not None:
        cfg.upsample_prompt_mode = upsample

    if remote is not None:
        if single_eval:
            success = generate_image_remote(cfg, remote)
            ...
            while True:
                ...
                if line in ("quit", "q", "exit"):
                    ...
                    print_config(cfg)
                elif line == "" or line == "run":
                    generate_image_remote(cfg, remote)
                else:
                    cfg.prompt = line
                    generate_image_remote(cfg, remote)
        return
    ...
    while True:
        ...
        if line in ("quit", "q", "exit"):
            ...
            if validate_model_params(model_name, cfg):
                print_config(cfg)
        elif line == "" or line == "run":
            try:
                generate_image(
